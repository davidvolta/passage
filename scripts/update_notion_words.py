...
def main():
    # Run ingest
    result = subprocess.run([sys.executable, str(SCRIPTS_DIR / "notion_ingest.py")], capture_output=True, text=True)
    ...
    # Generate words
    result = subprocess.run([sys.executable, str(SCRIPTS_DIR / "words.py"), "--collection", "notion_words", "--count", "50"], capture_output=True, text=True)
